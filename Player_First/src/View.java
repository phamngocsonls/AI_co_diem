					...
					if(selectedPiece.checkLegalMove(e.getPoint(), View.this.board)){ //check legal moves
						...
						//check for castling here
							...
							View.this.board.clearSquare(oldX, oldY);
                                                        if(selectedPiece.getColor() == false && newX == 3 && newY == 0 ){
                                                            View.this.board.clearSquare(newX, newY);
                                                            newX = 0;
                                                            newY = 7;
                                                           
                                                        }
                                                        else if(selectedPiece.getColor() == false && newX == 4 && newY == 0){
                                                            View.this.board.clearSquare(newX, newY);
                                                            newX = 7;
                                                            newY = 7;
                                                           
                                                        }
                                                        else if(selectedPiece.getColor() == true && newX == 3 && newY == 7){
                                                            View.this.board.clearSquare(newX, newY);
                                                            newX = 0;
                                                            newY = 0;
                                                           
                                                        }
                                                        else if(selectedPiece.getColor() == true && newX == 4 && newY == 7){
                                                            View.this.board.clearSquare(newX, newY);
                                                            newX = 7;
                                                            newY = 0;
                                                        }
	...
	public void paintComponent(Graphics g){
		...
		for(int i = 0; i<8; i++){
			...
			for(int j = 0; j<8; j++){
				...
				if(isBlack){	
					g2.setColor(new Color(113,198,113));
					...
					g2.setColor(Color.white);
                                ...
                                if( i == 3 && j == 0){
                                    try {
                                        img = ImageIO.read(new File("blackone.png"));
                                    } catch (IOException ex) {
                                        Logger.getLogger(View.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                    g2.drawImage(img, i*62, j*62, this);
                                }
                                  else if( i == 4 && j == 0){
                                    try {
                                        img = ImageIO.read(new File("blacktwo.png"));
                                    } catch (IOException ex) {
                                        Logger.getLogger(View.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                    g2.drawImage(img, i*62, j*62, this);
                                    ...
                                    try {
                                        img = ImageIO.read(new File("whitetwo.png"));
                                    } catch (IOException ex) {
                                        Logger.getLogger(View.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                    g2.drawImage(img, i*62, j*62, this);
                                    ...
                                    try {
                                        img = ImageIO.read(new File("whiteone.png"));
                                    } catch (IOException ex) {
                                        Logger.getLogger(View.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                    g2.drawImage(img, i*62, j*62, this);
