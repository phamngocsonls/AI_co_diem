...
public class Knight extends Piece {
	...
	public boolean checkLegalMove(Point p, Board b) {
		...
		if(Math.abs(newX - oldX) == 2 && Math.abs(newY - oldY) == 1 && (newX >= 0 && newX <= 7) && (newY >= 0 && newY <= 7)){
			if((newX < oldX) && (b.hasPiece(oldX - 1, oldY)))
                            return false;
                        else if((newX > oldX) && (b.hasPiece(oldX + 1, oldY)))
                            return false;
                        else
                        {
                        ...
                        if((newY < oldY) && (b.hasPiece(oldX , oldY - 1)))
                            return false;
                        else if((newY > oldY) && (b.hasPiece(oldX , oldY + 1)))
                            return false;
                        else
                        {
                             System.out.println("Legal move: " + getType()  + " from: (" + oldX + "," + oldY + ") to (" + newX + "," + newY + ")");
